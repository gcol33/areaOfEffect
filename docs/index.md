...
**Classify points as inside, near, or too far from a polygon boundary,
with a buffer width that picks itself.**

Give it points and a region. `areaOfEffect` labels each point **core**
(inside), **halo** (in the buffer zone), or prunes it (too far), and it
sizes the buffer for you: the default produces equal core and halo
areas, a scale-free definition of “near the boundary” that means the
same thing for Luxembourg and Brazil. Projection, buffering, and
point-in-polygon are handled through `sf` under one call.
...
The underlying `sf` workflow is well known: load a boundary, fix the
CRS,
...
The repetitive part is one function call. The harder part is the buffer
distance. A 10 km buffer is most of Luxembourg and a rounding error for
Brazil, so a fixed distance is not comparable across regions.

`areaOfEffect` solves for the distance instead of asking you to invent
one. The default scale `sqrt(2) - 1` is the unique value that makes the
halo area equal the core area: it is the solution of
`(1 + s)^2 - 1 = 1`, not a tuned constant. So “near the boundary” is
defined by the geometry of the region, the same way everywhere.
...
- **[`aoe()`](https://gcol33.github.io/areaOfEffect/reference/aoe.md)**:
  classify points as core, halo, or pruned against one or more supports.
  Pass a data frame with coordinates or an `sf` object; pass a country
  name (`"Austria"`, `"AT"`), your own polygon, or nothing (countries
  are detected from the points).
- **[`aoe_border()`](https://gcol33.github.io/areaOfEffect/reference/aoe_border.md)**:
  classify points by side and distance from a line, with symmetric
  equal-area zones on each side (for example, either side of an
  international border).
- **[`aoe_expand()`](https://gcol33.github.io/areaOfEffect/reference/aoe_expand.md)**:
  grow the buffer per support just enough to capture a minimum point
  count, under hard caps on area and distance.
- **[`aoe_sample()`](https://gcol33.github.io/areaOfEffect/reference/aoe_sample.md)**:
  stratified sampling of a result by core/halo or by side, for balanced
  draws when one class dominates.
- **[`aoe_summary()`](https://gcol33.github.io/areaOfEffect/reference/aoe_summary.md),
  ...
  counts and proportions per support, area statistics (halo:core ratio,
  masking effect), and the underlying AoE polygons for plotting.

## Solving for area after masking

The default equal-area buffer extends into anything around the region,
including ocean. For terrestrial data that area is wasted. The `mask`
argument clips the halo to relevant areas, and the `area` argument then
solves for the buffer that hits your target *after* clipping, so
equal-area still holds on the land that remains.
...
Without a mask the equal-area scale is analytic and exact. With one, the
masked area has no closed form, so the buffer is found by a short secant
search that converges in a handful of clipping evaluations.

## Scale

The `scale` argument sets halo size as a proportion of core area. The
default, `sqrt(2) - 1` (about 0.414), gives a 1:1 halo:core ratio.
