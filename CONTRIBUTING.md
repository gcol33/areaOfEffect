...
to the **areaOfEffect** project!

This document provides guidelines for contributing to areaOfEffect—its codebase and documentation.
...
Clone the areaOfEffect repository:

```bash
git clone https://github.com/gcol33/areaOfEffect.git
cd areaOfEffect
```
...
areaOfEffect is an R package that depends on the `sf` package for spatial operations.
...
2. **Install development dependencies**
   ```r
   install.packages(c("devtools", "roxygen2", "testthat", "rmarkdown", "knitr", "pkgdown", "sf"))
   ```
...
areaOfEffect uses **testthat** for testing.
...
testthat::test_file("tests/testthat/test-aoe.R")
```
...
└── areaOfEffect.Rcheck/    <- Artifacts from local package checks (ignored)
```
...
By contributing to areaOfEffect, you agree that your code is released under the same license as the package.
